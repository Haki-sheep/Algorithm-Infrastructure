...
{
    // Unity 2022 predates the EditorAnalytics IAnalytic/AnalyticInfo API these event classes are
    // written against (added in Unity 6); minimal stand-ins keep them compiling here, and the send
    // indirection below drops the events instead of shipping them.
    interface IAnalytic
    {
        interface IData { }
    ...
    sealed class AnalyticInfoAttribute : Attribute
    {
        public AnalyticInfoAttribute(string eventName, string vendorKey, int maxEventsPerHour, int maxNumberOfElements)
        {
        }
    }
}
...
namespace Unity.Pipeline.Editor
{
    ...
    internal static class PipelineAnalytics
    {
        ...
        /// </summary>
        internal static Action<IAnalytic> s_Send = SendAnalytic;

        private static void SendAnalytic(IAnalytic analytic)
        {
#if UNITY_6000_0_OR_NEWER
            EditorAnalytics.SendAnalytic(analytic);
#else
            // Unity 2022 has no EditorAnalytics.SendAnalytic; the legacy SendEventWithLimit API needs
            // a separately registered schema, so pipeline analytics are editor-only on 2022.
            _ = analytic;
#endif
        }
