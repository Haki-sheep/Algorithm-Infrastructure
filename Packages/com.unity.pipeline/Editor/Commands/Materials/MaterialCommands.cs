...
namespace Unity.Pipeline.Editor.Commands.Materials
{
    ...
    static class MaterialCommands
    {
        ...
        /// <summary>
        /// Read the render queue override (-1 = inherit from the shader). Unity 6 exposes this as the
        /// public <c>Material.rawRenderQueue</c>; on Unity 2022 the accessor is internal and the public
        /// <c>Material.renderQueue</c> resolves the shader queue when there is no override, so read the
        /// internal property to preserve the renderQueue:-1 round-trip (falling back if it is absent).
        /// </summary>
        private static int GetRawRenderQueue(Material mat)
        {
            var raw = typeof(Material).GetProperty("rawRenderQueue", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            return raw != null ? (int)raw.GetValue(mat) : mat.renderQueue;
        }
