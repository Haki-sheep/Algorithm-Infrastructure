...
namespace Unity.Pipeline.Editor.Console
{
    ...
    static class EditorConsoleGroundTruth
    {
        ...
        // Unity 2022 predates the public ConsoleWindowUtility; LogEntries (the store ConsoleWindow
        // itself reads) exposes the same per-severity counts, so read it through reflection there.
        static void GetConsoleLogCounts(out int errors, out int warnings, out int logs)
        {
            ...
            if (s_GetCountsByType == null)
            {
                ...
                s_GetCountsByType = logEntries?.GetMethod("GetCountsByType",
...
#if !UNITY_6000_0_OR_NEWER
        static System.Reflection.MethodInfo s_GetCountsByType;
#endif
